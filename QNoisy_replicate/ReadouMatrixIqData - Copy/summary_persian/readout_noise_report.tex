...
در پروژه، مدار \lr{ZZ} با تابع استاندارد \lr{zz\_feature\_map} ساخته می‌شود و پارامترهای $\theta_i$ با \lr{assign\_parameters} تزریق می‌شوند. نمونه زیر الگوی چهارکیوبیتی پروژه را نشان می‌دهد؛ برای مرور سریع بعدی، شناسه‌های اصلی در کامنت‌های انگلیسی توضیح داده شده‌اند.
...
در پروژه چهارکیوبیتی، همین الگو در حلقه \lr{for q in range(4)} تکرار می‌شود. برای نویز \lr{experimental\_single}، ماتریس‌های $A^{(q)}$ از فایل‌های \lr{assignment\_matrix\_q*.txt} بارگذاری می‌شوند. توجه: ماتریس همبسته $16\times16$ با این روش قابل اعمال مستقیم نیست (بخش بعد).
