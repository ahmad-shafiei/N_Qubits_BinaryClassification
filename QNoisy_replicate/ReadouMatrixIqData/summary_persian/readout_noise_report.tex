...
در ارزیابی نهایی، هر مدل آموزش‌دیده فقط روی همان نویزی آزمون می‌شود که با آن آموزش دیده است. جدول~\ref{tab:main-fidelity} baseline (بدون آموزش) را در برابر مدل آموزش‌دیده مقایسه می‌کند.
...
& Exp. Single & 0.768 & 0.977 & +0.209\\
& Exp. Correlated & 0.763 & 0.971 & +0.208\\
\midrule
\multirow{3}{*}{ZZ FeatureMap}
& Synthetic & 0.938 & 0.972 & +0.034\\
& Exp. Single & 0.853 & 0.934 & +0.081\\
& Exp. Correlated & 0.861 & 0.963 & +0.102\\
\bottomrule
\end{tabular}
\end{table}
...
ردیف baseline نشان می‌دهد نویز به تنهایی چقدر توزیع ایده‌آل را خراب می‌کند. برای مدار مستقل، fidelity نویز مصنوعی $0.907$ است، اما برای نویزهای تجربی به حدود $0.76$ افت می‌کند. این نشان می‌دهد نویز تجربی بسیار جدی‌تر از flip مصنوعی ساده است. برای مدار \lr{ZZ FeatureMap} baselineها کمی بالاترند؛ شکل توزیع این مدار با همین نویزها overlap بیشتری با ایده‌آل حفظ می‌کند.

\subsection{اثر آموزش با نویز منطبق}

وقتی train و test روی همان کانال نویز باشند، مدل \lr{MLP} به‌طور پایدار fidelity را بالا می‌برد:
\begin{itemize}
...
\end{itemize}
این الگو نشان می‌دهد مدل واقعاً نگاشت معکوس کانال readout را یاد گرفته است. بیشترین بهبود ($\Delta F \approx 0.21$) برای نویزهای تجربی مدار مستقل است؛ جایی که baseline پایین‌تر بوده و فضای بیشتری برای mitigation وجود داشته.

\subsection{چرا ارزیابی منطبق؟}

در نسخه اولیه پروژه، هر مدل روی هر سه نویز آزمون می‌شد (ماتریس $4\times3$). آن رویکرد برای مطالعه \lr{robustness} مفید بود، اما تحلیل را پیچیده می‌کرد: گاهی مدل آموزش‌دیده روی نویز A وقتی روی نویز B آزمون می‌شد، \emph{از baseline بدتر} عمل می‌کرد (مثلاً \lr{ZZ/Exp. Single} روی \lr{Synthetic}: fidelity $0.848$ در برابر baseline $0.938$). بنابراین در نسخه نهایی، ارزیابی فقط در حالت منطبق انجام می‌شود تا:
\begin{enumerate}
	\item سنجش مستقیم «آیا مدل کانال نویز را invert کرده؟»؛
	\item مقایسه منصفانه baseline در برابر trained؛
	\item گزارش ساده‌تر با جدول baseline vs trained برای هر $(\text{مدار}, \text{نویز})$.
\end{enumerate}

\subsection{بهبود نسبت به baseline}

جدول~\ref{tab:main-fidelity} نشان می‌دهد بیشترین سود یادگیری ماشین در جایی است که نویز تجربی شدیدتر بوده است. در مدار مستقل، نویز تجربی fidelity را حدود $0.76$ کرده و مدل آن را تا حدود $0.97$ برگردانده است ($\Delta F \approx 0.21$). در مدار \lr{ZZ} baselineها بالاترند، اما مدل همچنان بهبود معنادار ($\Delta F \approx 0.03$ تا $0.10$) ایجاد می‌کند.
...
\begin{enumerate}
	...
	\item \textbf{مدل mitigation باید با شرایط آزمون سازگار باشد}: train و test باید روی همان کانال نویز باشند؛ در غیر این صورت حتی از baseline هم بدتر می‌شود.
\end{enumerate}
...
در بخش دوم، سه نوع نویز روی دو مدار کوانتومی اعمال می‌شود. ارزیابی نهایی با \textbf{نویز منطبق train/test} انجام می‌شود: هر مدل فقط روی همان کانال نویزی آزمون می‌شود که با آن آموزش دیده است. مدل \lr{MLP} نقش تقریب کانال معکوس readout را دارد و با fidelity، \lr{KL} و \lr{L1} ارزیابی می‌شود.

نتایج نشان می‌دهد mitigation در حالت منطبق بسیار مؤثر است، به‌ویژه برای نویز تجربی. baseline vs trained به‌صورت مستقیم قابل مقایسه است. برای کاربرد سخت‌افزاری، استفاده از ماتریس همبسته واقعی و آموزش/آزمون روی همان کانال نویز ضروری است.
