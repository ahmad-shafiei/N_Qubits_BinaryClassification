\chapter*{گزارش فنی پروژه نویز خوانش چند کیوبیتی}
...
\begin{itemize}
	\item ماژول های خوانش داده ها  استخراج ماتریس های نویز: بارگذاری داده‌های تجربی، آموزش تفکیک‌کننده \lr{IQ}، استخراج ماتریس‌های $2\times2$ و $16\times16$؛
	\item ماژول های تولید و ذخیره دیتاست و مدل ها: ساخت مدار، اعمال سه نوع نویز، تولید دیتاست، آموزش \lr{MLP} و ارزیابی با fidelity، \lr{L1} و \lr{KL}.
\end{itemize}

\textbf{اجرای خط لوله (خلاصه):}
\begin{enumerate}
	\item بررسی داده \lr{IQ} ؛
	\item استخراج ماتریس؛
	\item تولید دیتاست؛
	\item آموزش و ارزیابی در.
\end{enumerate}

در نسخه فعلی پروژه، همه مسیرهای ورودی/خروجی به \emph{snapshot} (مثلاً \lr{1\_1\_2025}) وابسته‌اند تا چند کمپین اندازه‌گیری تجربی بتوانند هم‌زمان نگهداری و مقایسه شوند.
...
\subsection*{ساختار snapshot و مسیرهای پروژه}

برای توسعه به سمت چند مجموعه داده تجربی، خط لوله به صورت سلسله‌مراتبی سازماندهی شده است:


هر اسنپ شات یک کمپین اندازه‌گیری مستقل است. ماتریس نویز استخراج‌شده از فقط روی دیتاست همان اسنپ شات اعمال می‌شود. این جداسازی برای مقایسه drift سخت‌افزار، افزودن داده جدید بدون بازنویسی کد، و تکرارپذیری آزمایش‌ها ضروری است.

\section*{داده‌های تجربی \lr{IQ}}

\subsection*{ساختار فایل‌ها}

داده‌های تجربی در پوشه  مربوطه قرار دارند. برای هر snapshot و برای هر یک از ۱۶ حالت پایه محاسباتی یک فایل وجود دارد؛ از \lr{0000.txt} تا \lr{1111.txt}.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.62\textwidth]{guassian_ellipses.png}
\caption{بیضی‌های گاوسی برای کیوبیت ۳ در دو حالت آماده‌سازی \lr{0100} (آبی) و \lr{1001} (قرمز). محور افقی $I$ و محور عمودی $Q$. همپوشانی ابرها و بیضی‌ها نشان می‌دهد چرا \lr{LDA} در برخی ناحیه‌ها شات را اشتباه طبقه‌بندی می‌کند و fidelity خوانش از یک کاهش می‌یابد.}
\label{fig:gaussian-ellipses}
\end{figure}

شکل~\ref{fig:gaussian-ellipses} فرض توزیع گاوسی \lr{LDA} را به‌صورت بصری تأیید می‌کند: هر حالت آماده‌سازی یک ابر متمرکز دارد، اما مراکز و پراکندگی‌ها برای حالت‌های مختلف چهارکیوبیتی متفاوت است---پیش‌زمینه‌ای برای نیاز به ماتریس همبسته $16\times16$.
...
\caption{مدار \lr{ZZFeatureMap} چهارکیوبیتی. این مدار شامل لایه‌های هادامارد، گیت‌های فاز و بلوک‌های \lr{ZZ} بین جفت کیوبیت‌هاست و توزیع احتمال غیرقابل فاکتورسازی تولید می‌کند.}
\label{fig:zz-circuit}
\end{figure}

مدار دوم با تابع \lr{zz\_feature\_map} در \lr{Qiskit} ساخته شده است. نقش گیت‌ها چنین است:
\begin{itemize}
	\item \textbf{Hadamard}: حالت $|0\rangle$ را به $|+\rangle$ می‌برد و برهم‌نهی  اولیه ایجاد می‌کند؛
...
\subsection{نمونه کد: ساخت مدار \lr{ZZFeatureMap}}

در پروژه، مدار \lr{ZZ} با تابع استاندارد \lr{zz\_feature\_map} ساخته می‌شود و پارامترهای $\theta_i$ تزریق می‌شوند. نمونه زیر الگوی چهارکیوبیتی پروژه را نشان می‌دهد؛ برای مرور سریع بعدی، شناسه‌های اصلی در کامنت‌های انگلیسی توضیح داده شده‌اند.

\code{python}{snippets/qiskit_zz_featuremap_4q.py}{code:zz-featuremap}

در این پروژه \lr{reps=2} برای ایجاد تنوع کافی در توزیع خروجی انتخاب شده است (بخش~\ref{tab:challenges}). حالت \lr{entanglement="full"} یعنی برهم‌کنش \lr{ZZ} بین همه جفت کیوبیت‌ها در هر لایه فعال است.
...
\subsection*{نمونه کد: \lr{ReadoutError} و \lr{NoiseModel} در \lr{Aer}}

برای نویزهای \lr{synthetic} و \lr{experimental\_single}، خطای خوانش تک‌کیوبیتی با \lr{ReadoutError} به \lr{NoiseModel} اضافه می‌شود و شبیه‌سازی نویزی از طریق \lr{AerSimulator} انجام می‌گیرد. نمونه دوکیوبیتی زیر قرارداد پروژه را نشان می‌دهد: ماتریس $A$ سطری است ($A_{ij}=P(m=j\mid p=i)$) و به هر کیوبیت جداگانه وصل می‌شود.

\code{python}{snippets/qiskit_readout_noise_2q.py}{code:readout-noise-2q}

در پروژه چهارکیوبیتی، همین الگو در یک حلقه تکرار می‌شود. توجه: ماتریس همبسته $16\times16$ با این روش قابل اعمال مستقیم نیست (بخش بعد).

\subsection*{نویز تجربی تک‌کیوبیتی}

در حالت \lr{experimental\_single}، چهار ماتریس $A^{(q)}$ که در بخش اول استخراج شده‌اند بارگذاری می‌شوند و برای هر کیوبیت به صورت مستقل به \lr{NoiseModel} داده می‌شوند (الگوی کد در بخش~\ref{code:readout-noise-2q}).
...
\begin{enumerate}
	...
	\]
\end{enumerate}
هر بردار احتمال ۱۶ بعدی است و ترتیب مؤلفه‌ها مطابق حالت‌های \lr{0000} تا \lr{1111} است. دیتاست‌ها در ساختار مناسب ذخیره می‌شوند. در اجرای اصلی snapshot \lr{1\_1\_2025}، اندازه‌ها برای train، validation و test به ترتیب ۴۵۰۰، ۱۵۰۰ و ۱۵۰۰ نمونه بوده است.
...
\begin{itemize}
	\item  classifier حالت $|s\rangle$  نیست ؛
	\item  state discriminator روی \lr{IQ} نیست ؛
	\item تقریب غیرخطی از \emph{inverse readout channel} هست:
	\[
...
\subsection{\lr{Fidelity} (وفاداری / شباهت \lr{Bhattacharyya-based})}
...
از دید فیزیکی، fidelity میزان همپوشانی دو توزیع را می‌سنجد. اگر نویز خوانش بخشی از احتمال را از حالت‌های درست به حالت‌های اشتباه منتقل کند، هم‌پوشی کم می‌شود. اگر مدل mitigation موفق باشد، $F(\mathbf{p}_{\mathrm{ideal}}, \hat{\mathbf{p}})$ افزایش می‌یابد.

\subsection*{\lr{L1 Error} (فاصله منهتن / \lr{Total Variation-like})}
...
ساختار flat & یک snapshot، مسیرهای ثابت & معماری \lr{snapshot} + CLI \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection*{نمونه تک‌نمونه‌ای بازسازی توزیع}

جدول~\ref{tab:main-fidelity} و نمودارهای تجمیعی، میانگین روی ۱۵۰۰ نمونه test را گزارش می‌کنند. برای درک بصریِ آنچه مدل انجام می‌دهد، تابع \lr{visualize\_aligned\_samples} روی یک نمونه test، سه توزیع ۱۶‌حالته را کنار هم رسم می‌کند: ایده‌آل (\lr{Ideal})، نویزی (\lr{Noisy}) و خروجی \lr{MLP} (\lr{Predicted}). شکل~\ref{fig:sample-zz-corr} خروجی این دستور برای فاز \lr{3\_2} است. یعنی مدار \lr{ZZ FeatureMap}، نویز آموزش/آزمون \lr{Experimental Correlated}، و یک نمونه تصادفی از دیتاست \lr{test}.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{visualize_aligned_fidelity_zz_corr_.png}
\caption{بازسازی توزیع برای یک نمونه test: \lr{ZZ FeatureMap} با نویز همبسته تجربی. میله‌های آبی ایده‌آل، نارنجی نویزی، سبز پیش‌بینی \lr{MLP}. fidelity این نمونه $F \approx 0.87$.}
\label{fig:sample-zz-corr}
\end{figure}

در این نمونه، بیشینه احتمال روی حالت‌های $12$ و $5$ متمرکز است. نویز خوانش بخشی از جرم احتمال را از این حالت‌ها به حالت‌های دیگر منتقل کرده (میله‌های نارنجی پهن‌تر و کوتاه‌تر از آبی). خروجی \lr{MLP} (سبز) در اکثر اندیس‌ها به میله آبی نزدیک‌تر است؛ به‌ویژه در حالت $12$ که بیشینه اصلی است، پیش‌بینی تقریباً ایده‌آل را بازمی‌گرداند. این همان چیزی است که شکل مفهومی~\ref{fig:dist-reconstruction} به صورت شماتیک نشان می‌داد، اما اینجا روی داده واقعی test و کانال نویز ۱۶$\times$۱۶ همبسته است. fidelity نمونه ($\approx 0.87$) از میانگین کل ($0.974$ در جدول) پایین‌تر است که طبیعی است: برخی نمونه‌ها سخت‌تر از میانگین بازسازی می‌شوند.
...
\section*{به‌روزرسانی زیرساخت برای توسعه و چند snapshot}

در طول این پروژه، علاوه بر نتایج علمی، \textbf{زیرساخت نرم‌افزاری} نیز بازطراحی شد تا بتوانیم چند کمپین اندازه‌گیری تجربی را بدون تکرار دستی آزمایش‌ها مدیریت کنیم. انگیزه اصلی این کار، آماده‌سازی مسیر توسعه به سمت داده بیشتر، مقایسه drift سخت‌افزار، و در نهایت تبدیل این گزارش به مقاله قابل چاپ است.

\subsection*{چرا این تغییرات برای بهبود نتایج مؤثر است؟}

\begin{enumerate}
	\item \textbf{داده تجربی بیشتر:} هر snapshot جدید (مثلاً روز یا تنظیمات کالیبراسیون متفاوت) بدون شکستن کد قبلی اضافه می‌شود؛ می‌توان mitigation را روی چند ماتریس نویز مقایسه کرد.
	\item \textbf{هم‌ترازی فیزیکی:} ماتریس نویز و دیتاست \lr{ML} همیشه از یک snapshot می‌آیند؛ خطای ناشی از جفت‌کردن اشتباه ماتریس و داده حذف می‌شود.
	\item \textbf{تکرارپذیری:} دستورات \lr{run\_readout\_matrix.py} و \lr{build\_ml\_datasets.py} خط لوله را مستند و قابل اجرای batch می‌کنند---الزام مقالات علمی.
	\item \textbf{مقیاس‌پذیری:} با افزایش تعداد snapshotها، می‌توان مدل mitigation را روی داده ترکیبی یا چند سناریو drift ارزیابی کرد و robustness را سنجید.
	\item \textbf{جداسازی concernها:} استخراج نویز (Part 1) از تولید دیتاست و آموزش (Part 2) جدا شده؛ هر فاز مستقل قابل اجرا و debug است.
\end{enumerate}

\subsection*{چارچوب پیشنهادی برای مقاله}

ساختار این گزارش عمداً با بخش‌بندی یک مقاله علمی هم‌راستا است:

\begin{table}[htbp]
\centering
\caption{نگاشت بخش‌های گزارش به ساختار مقاله}
...
\midrule
داده \lr{IQ} و \lr{LDA} & Methods: Experimental setup \& Readout calibration \\
ماتریس $2\times2$ و $16\times16$ & Methods: Noise matrix extraction \& validation \\
مدارها و سه نوع نویز & Methods: Circuit families \& noise models \\
استراتژی فازها و دیتاست & Methods: Dataset design \& training protocol \\
جدول~\ref{tab:main-fidelity} و نمودارها & Results: Quantitative mitigation performance \\
محدودیت‌ها و توسعه snapshot & Discussion \& Future work \\
\bottomrule
\end{tabular}
\end{table}

گام بعدی برای انتشار: افزودن snapshotهای بیشتر، تحلیل آماری اطمینان (مثلاً \lr{bootstrap} روی \lr{test})، و در صورت امکان مقایسه با روش‌های کلاسیک \lr{mitigation} (مثلاً معکوس ماتریس منظم‌شده).
...
\begin{enumerate}
	...
	\item \textbf{وابستگی به کالیبراسیون:} ماتریس‌های نویز وابسته به snapshot فعلی (\lr{1\_1\_2025}) هستند؛ معماری جدید امکان افزودن snapshotهای بعدی را فراهم کرده، اما در این گزارش فقط یک snapshot تحلیل شده است.
	\item \textbf{ارزیابی منطبق:} در این گزارش فقط حالت train/test matched بررسی شده؛ تعمیم به نویز یا مدار دیگر در scope فعلی نیست.
	\item \textbf{تقریب غیرخطی:} \lr{MLP} معکوس دقیق $M$ نیست؛ outside distribution ممکن است ضعیف عمل کند.
	\item \textbf{نویز فقط \lr{readout} هست:} \lr{gate error} و \lr{decoherence} در شبیه‌سازی لحاظ نشده‌اند.
\end{enumerate}
...
این پروژه یک مسیر کامل و قابل دفاع برای مطالعه نویز خوانش چهار کیوبیتی ساخته است (شکل~\ref{fig:pipeline}). در بخش اول، داده‌های خام \lr{IQ}  با \lr{LDA} به بیت‌های اندازه‌گیری‌شده تبدیل می‌شوند و پس از اعتبارسنجی، ماتریس‌ها ذخیره می‌گردند (شکل‌های~\ref{fig:iq-clouds} و~\ref{fig:gaussian-ellipses}).

در بخش دوم، سه نوع نویز روی دو مدار کوانتومی اعمال می‌شود و دیتاست‌ها در \lr{DataTrain/<snapshot>/} نگهداری می‌شوند. ارزیابی نهایی \textbf{فقط} با نویز منطبق train/test انجام می‌شود. مدل \lr{MLP} نقش تقریب کانال معکوس readout را دارد و با fidelity، \lr{KL} و \lr{L1} ارزیابی می‌شود (جدول~\ref{tab:main-fidelity}، شکل‌های~\ref{fig:fidelity-ind}--\ref{fig:metrics-zz} و نمونه تک‌نمونه‌ای شکل~\ref{fig:sample-zz-corr}).

نتایج نشان می‌دهد mitigation در حالت منطبق بسیار مؤثر است: بیشترین بهبود fidelity ($\Delta F \approx 0.22$) برای مدار مستقل و نویز همبسته است؛ در \lr{ZZ FeatureMap} fidelity نهایی در هر سه نویز بالای $0.93$ باقی می‌ماند. بازطراحی زیرساخت مبتنی بر \lr{snapshot}  پایه‌ای برای گسترش به داده تجربی بیشتر و تبدیل این کار به مقاله علمی فراهم کرده است (جدول~\ref{tab:paper-mapping}).
